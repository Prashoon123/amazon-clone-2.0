import React, { useState } from "react";
import { StarIcon } from "@heroicons/react/solid";
import Image from "next/image";
import Currency from "react-currency-formatter";
import { useDispatch } from "react-redux";
import { addToBasket } from "../slices/basketSlice";
import { useRouter } from "next/router";

function ProductInfo({ id, title, price, description, category, image }) {
  const router = useRouter();
  const dispatch = useDispatch();
  const [rating] = useState(Math.floor(Math.random() * 5));
  const [hasPrime] = useState(Math.random() < 0.5);

  const addItemToBasket = () => {
    const product = {
      id,
      title,
      price,
      description,
      category,
      image,
      hasPrime,
      rating,
    };

    dispatch(addToBasket(product));
  };

  const placeholderProductImg =
    "data:image/svg+xml;base64,";

  return (
    <div className="flex flex-col w-screen">
      <center>
        <button
          onClick={() => router.push("/")}
          className="w-52 button mt-10 from-black to-black border-white text-white active:from-white active:to-white active:text-black active:border-black"
        >
          Return to homepage
        </button>
      </center>
      <div className="flex flex-col mx-20 mt-8 items-center bg-white shadow-md p-10 rounded-md">
        <Image
          src={image || placeholderProductImg}
          height={200}
          width={200}
          objectFit="contain"
        />

        <div className="h-5" />

        <div className="col-span-3 mx-5">
          <p>{title}</p>

          <div className="h-2" />

          <div className="flex">
            {Array(rating || Math.floor(Math.random() * 4))
              .fill()
              .map((_, i) => (
                <StarIcon key={i} className="h-5 text-yellow-500" />
              ))}
          </div>

          <p className="text-xs my-2 line-clamp-3">{description}</p>

          <Currency quantity={price} currency="USD" />

          {hasPrime && (
            <div className="flex items-center space-x-2">
              <img
                src="https://links.papareact.com/fdw"
                className="w-12"
                loading="lazy"
              />
              <p className="text-xs text-gray-500">FREE Next-day Delivery</p>
            </div>
          )}
        </div>

        <div className="flex flex-col space-y-2 my-auto">
          <button className="button w-52" onClick={addItemToBasket}>
            Add to Cart
          </button>
        </div>
      </div>
    </div>
  );
}

export default ProductInfo;
